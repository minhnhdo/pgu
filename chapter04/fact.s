...
# This program has no global data
 .section .text
 .code32              # sets the assembler to 32 bit mode
 .globl _start
...
_start:
 pushl $4             # The factorial takes one argument, the number
                      # we want the factorial of, so it gets pushed.
 call factorial       # run the factorial function
 addl $4, %esp        # scrub the factorial function we pushed onto
                      # the stack
 movl %eax, %ebx      # factorial returns the answer in %eax, but
                      # we want it in %ebx to send it as out exit
                      # status
...
factorial:
 pushl %ebp           # standard function stuff - we have to restore
                      # %ebp to its prior state before returning, so
                      # we have to push it
 movl %esp, %ebp      # This is because we don't want to modify the
 ...
 call factorial
 movl 8(%ebp), %ebx   # %eax has the return value, so we reload our
                      # parameter into %ebx
...
end_factorial:
 movl %ebp, %esp      # standard function return stuff - we have to
 popl %ebp            # restore %ebp and %esp to where they were
